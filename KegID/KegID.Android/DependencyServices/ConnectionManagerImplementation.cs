...
using KegID.Android.DependencyServices;
...
namespace KegID.Android.DependencyServices
{
    public class ConnectionManagerImplementation : IConnectionManager
    {
        ...
        public Connection GetUsbConnection(string symbolicName)
        {
            lock (UsbConnectionLock)
            {
                try
                {
                    UsbManager usbManager = (UsbManager)global::Android.App.Application.Context.GetSystemService(Context.UsbService);
                    ...
                    if (usbDevice != null)
                    {
                        ...
                        {
                            PendingIntent permissionIntent = PendingIntent.GetBroadcast(global::Android.App.Application.Context, 0, new Intent(ActionUsbPermission), 0);
